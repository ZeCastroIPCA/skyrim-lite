...
export class FightComponent implements OnInit {
  ...
  getPlayer(id : string) : void{
    this.service.getID(id).subscribe((x) => {
      if (x['code'] == 200) {
        this.service.player.id = x['data'].ID
        this.service.player.idPersonagem = x['data'].ID_Player
        this.service.player.name = x['data'].Nome
        this.service.player.stats.health = x['data'].Vida
        this.service.player.stats.damage = x['data'].Atk
        this.service.player.stats.inteligence = x['data'].Int
      }
      console.log(x)
    });
  }
  ...
  attack() : void{
    if(this.getDamage() == 'failed'){
      if (this.count % 2 == 0) {
        this.service.enemy.stats.health = this.service.enemy.stats.health
      } else {
